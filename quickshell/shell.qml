...
ShellRoot {
    ...
    property string btPairingDeviceName: ""
    property string btPairingPasskey: ""
    property bool btConfirmDialogVisible: false
    ...
    function refreshBluetooth() {
        console.log("[BT DEBUG] refreshBluetooth called")
        root.btPairedDevices = []
        root.btAvailableDevices = []
        root.btScanning = false
        root.btConnectingMAC = ""
        console.log("[BT DEBUG] Starting btStatusProc")
        if (!btStatusProc.running) btStatusProc.running = true
    }

    function connectBt(mac) {
        console.log("[BT CONNECT] Starting connect for MAC: " + mac)
        root.btConnectingMAC = mac
        btActionProc.command = ["bash", "-c", "{ echo \"trust " + mac + "\"; sleep 1; echo \"connect " + mac + "\"; sleep 2; echo \"quit\"; } | bluetoothctl 2>&1"]
        console.log("[BT CONNECT] Running connect command")
        btActionProc.running = true
    }

    function disconnectBt(mac) {
        console.log("[BT DISCONNECT] Starting disconnect for MAC: " + mac)
        btActionProc.command = ["bash", "-c", "{ echo \"disconnect " + mac + "\"; sleep 1; echo \"quit\"; } | bluetoothctl 2>&1"]
        console.log("[BT DISCONNECT] Running disconnect command")
        btActionProc.running = true
    }

    function pairBt(mac) {
        console.log("[BT PAIR] Starting pair for MAC: " + mac)
        // Find device name
        var deviceName = mac
        for (var i = 0; i < root.btAvailableDevices.length; i++) {
            if (root.btAvailableDevices[i].mac === mac) {
                deviceName = root.btAvailableDevices[i].name
                break
            }
        }
        root.btConnectingMAC = mac
        root.btPairingDeviceName = deviceName
        console.log("[BT PAIR] Device: " + deviceName)
        
        // Use helper script for interactive pairing
        btActionProc.command = ["bash", "-c", "chmod +x '" + configPath + "/bt-pair-helper.sh' && '" + configPath + "/bt-pair-helper.sh' '" + mac + "' 2>&1"]
        console.log("[BT PAIR] Starting pairing process")
        btActionProc.running = true
        
        // Start monitor for passkey file
        btPasskeyMonitorProc.running = true
    }
    
    function confirmBtPasskey(confirm) {
        console.log("[BT PAIR] User response: " + (confirm ? "yes" : "no"))
        root.btConfirmDialogVisible = false
        
        // Write response to file that helper script is waiting for
        var cmd = "echo '" + (confirm ? "yes" : "no") + "' > /tmp/bt-response"
        writeResponseProc.command = ["bash", "-c", cmd]
        writeResponseProc.running = true
    }
    
    function readPasskeyFile() {
        readPasskeyProc.command = ["bash", "-c", "cat /tmp/bt-passkey 2>/dev/null || echo ''"]
        readPasskeyProc.running = true
    }
    ...
    function enableBtDiscoverable() {
        console.log("[BT DISCOVERABLE] Enabling discoverable mode (180 seconds)")
        btActionProc.command = ["bash", "-c", "{ echo \"power on\"; sleep 1; echo \"discoverable on\"; sleep 1; echo \"pairable on\"; sleep 1; echo \"agent on\"; sleep 1; echo \"default-agent\"; sleep 1; echo \"quit\"; } | bluetoothctl 2>&1"]
        btActionProc.running = true
    }

    function setupBtAgent() {
        console.log("[BT AGENT] Setting up auto-confirming Bluetooth agent")
        btActionProc.command = ["bash", "-c", "python3 '" + configPath + "/bt-agent.py' 2>&1"]
        btActionProc.running = true
    }

    function startBtAgent() {
        console.log("[BT AGENT] Starting background Bluetooth agent")
        btAgentProc.command = ["bash", "-c", "python3 '" + configPath + "/bt-agent.py'"]
        btAgentProc.running = true
    }

    function stopBtAgent() {
        console.log("[BT AGENT] Stopping Bluetooth agent")
        btAgentProc.running = false
    }
    ...
    Process {
        ...
        // use connection profile approach to avoid "property is missing" errors
        command: {
            console.log("just a test log")
            var conName = "qshell-wifi-" + Math.random().toString(36).substr(2, 9)
            var innerCmd = ""
            if (password !== "") {
                innerCmd = "nmcli connection add type wifi con-name '" + conName + "' ifname '*' ssid '" + ssid.replace(/'/g, "'\"'\"'") + "' wifi-sec.key-mgmt wpa-psk wifi-sec.psk '" + password.replace(/'/g, "'\"'\"'") + "'"
            } else {
                innerCmd = "nmcli connection add type wifi con-name '" + conName + "' ifname '*' ssid '" + ssid.replace(/'/g, "'\"'\"'") + "'"
            }
            innerCmd += " && nmcli connection up '" + conName + "'"
            var header = "echo '=== wifi connect: " + (new Date()).toISOString() + " SSID: " + ssid.replace(/'/g, "'\"'\"'") + " ===' >> '" + root.statePath + "/wifi-connect.log'"
            return ["bash", "-c", header + " && (" + innerCmd + ") 2>&1 | tee -a '" + root.statePath + "/wifi-connect.log'"]
        }
        stdout: SplitParser {
            onRead: data => {
                console.log("wifiConnectProc:", data.trim())
            }
        }
        onExited: {
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: data => {
                var enabled = data.trim() === "true"
                console.log("[BT DEBUG] btStatusProc result: " + enabled)
                root.btEnabled = enabled
            }
        }
        onExited: {
            console.log("exited get called")
            console.log("[BT DEBUG] btStatusProc exited, btEnabled = " + root.btEnabled)
            if (root.btEnabled && !btDevicesProc.running) {
                console.log("[BT DEBUG] Starting btDevicesProc")
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: data => {
                ...
                console.log("[BT DEBUG] Found paired device: " + name + " (" + mac + ") connected=" + connected)
        ...
        onExited: {
            console.log("[BT DEBUG] btDevicesProc exited, found " + root.btPairedDevices.length + " paired devices")
            if (!btScanProc.running) {
                console.log("[BT DEBUG] Starting btScanProc for available devices")
    ...
    Process {
        ...
        command: ["bash", "-c",
            ...
            "echo -e 'scan off\\n'; " +
            "echo -e 'devices\\n'; " +
            "echo -e 'quit\\n') | bluetoothctl 2>/dev/null | grep '^Device' | while read -r line; do mac=$(echo \"$line\" | awk '{print $2}'); name=$(echo \"$line\" | cut -d' ' -f3-); info=$(echo -e \"info $mac\\nquit\" | bluetoothctl 2>/dev/null); paired=$(echo \"$info\" | grep -oP 'Paired: \\K\\w+'); if [ \"$paired\" != \"yes\" ] && [ -n \"$name\" ]; then echo \"${mac}|${name}\"; fi; done"
        ]
        stdout: SplitParser {
            onRead: data => {
                var line = data.trim()
                if (line.length === 0) return
                console.log("[BT DEBUG] Scan output: " + line)
                var parts = line.split("|")
                if (parts.length < 2) return
                var mac = parts[0]
                var name = parts[1]
                if (mac.length !== 17) {
                    console.log("[BT DEBUG] Invalid MAC address: " + mac)
                    return
                }
                console.log("[BT DEBUG] Found available device: " + name + " (" + mac + ")")
        ...
        onExited: {
            root.btScanning = false
            console.log("[BT DEBUG] btScanProc exited, found " + root.btAvailableDevices.length + " available devices")
        }
    }
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: data => {
                console.log("[BT ACTION] " + data.trim())
            }
        }
        onExited: {
            console.log("[BT ACTION] Process exited")
            root.btConnectingMAC = ""
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: data => {
                console.log("[BT AGENT] " + data.trim())
            }
        }
        onExited: {
            console.log("[BT AGENT] Agent process exited")
        }
    }

    Process {
        id: btPasskeyMonitorProc
        command: ["bash", "-c", "while [ -z \"$(cat /tmp/bt-passkey 2>/dev/null)\" ]; do sleep 0.1; done; cat /tmp/bt-passkey"]
        stdout: SplitParser {
            onRead: data => {
                var passkey = data.trim()
                if (passkey.length > 0) {
                    console.log("[BT PAIR] Passkey received: " + passkey)
                    root.btPairingPasskey = passkey
                    root.btConfirmDialogVisible = true
                    console.log("[BT PAIR] Showing confirmation dialog")
                    btConfirmDialog.show(root.btPairingDeviceName, passkey)
                }
            }
        }
        onExited: {
            console.log("[BT PASS]KEY] Passkey monitor ended")
        }
    }

    Process {
        id: writeResponseProc
        onExited: {
            console.log("[BT PAIR] Response written to file")
        }
    }
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: data => {
                var passkey = data.trim()
                if (passkey.length > 0) {
                    console.log("[BT PAIR] Passkey file contains: " + passkey)
                    root.btPairingPasskey = passkey
                    root.btConfirmDialogVisible = true
                    btConfirmDialog.show(root.btPairingDeviceName, passkey)
                }
    ...
    BluetoothConfirmDialog {
        id: btConfirmDialog
        onConfirm: root.confirmBtPasskey(true)
        onCancel: root.confirmBtPasskey(false)
    }
